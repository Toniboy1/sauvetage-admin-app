...
import { IAlarmExtended, IPropsAlarm } from "./types";
...
/**
 * Renders a component for managing alarm.
 * @param props - The component props.
 * @param props.labelText - The label text for the component.
 * @param props.allowCreate - A flag indicating whether to allow creating new alarm.
 * @returns The rendered component.
 */
const Alarm = ({ labelText, allowCreate = false }: IPropsAlarm) => {
  const [open, setOpen] = useState(false);
  const [options, setOptions] = useState<readonly IAlarmExtended[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchAlarm = async () => {
      ...
      const extendedOptions: IAlarmExtended[] = allAlarm.map((person) => ({
        id: person.id,
        name: person.name,
    ...
    <div>
      <div {...getRootProps()}>
        ...
        <InputWrapper ref={setAnchorEl} className={focused ? "focused" : ""}>
          {value.map((option: IAlarmExtended, index: number) => (
