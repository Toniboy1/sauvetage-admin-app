...
import { IInterventionExtended, IPropsIntervention } from "./types";
...
/**
 * Renders a component for managing intervention.
 * @param props - The component props.
 * @param props.labelText - The label text for the component.
 * @param props.allowCreate - A flag indicating whether to allow creating new intervention.
 * @returns The rendered component.
 */
const Intervention = ({ labelText, allowCreate = false }: IPropsIntervention) => {
  const [open, setOpen] = useState(false);
  const [options, setOptions] = useState<readonly IInterventionExtended[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchIntervention = async () => {
      ...
      const extendedOptions: IInterventionExtended[] = allIntervention.map((person) => ({
        id: person.id,
        name: person.name,
    ...
    <div>
      <div {...getRootProps()}>
        ...
        <InputWrapper ref={setAnchorEl} className={focused ? "focused" : ""}>
          {value.map((option: IInterventionExtended, index: number) => (
