 ...
 * @param form - The actions data
 * @param options - The action options
 * @param startingY - The starting Y position
 * @returns The new Y position
 */
const ActionTaken = (doc: jsPDF, form: IInterventionFormData, options: IAction[], startingY: number): number => {
  const rows = Math.ceil(options.length / 3);
  TITLE(doc);
  doc.text("Mesures prises:", 20, startingY + 10);
  TEXT_FONT(doc);
  let count = 0
  let x = 0
