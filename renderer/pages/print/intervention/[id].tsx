...
const TEXT_FONT = (doc:jsPDF)=>{ doc.setTextColor(0,0,0);;doc.setFont('calibri')}
const SUBTITLE_FONT = (doc:jsPDF)=>{ doc.setFontSize(25);doc.setFont('calibri-light')}
const TITLE = (doc:jsPDF)=>{ doc.setFontSize(15);doc.setFont('calibri-bold'); doc.setTextColor(0,0,255);}
const HEADER_TITLE = (doc:jsPDF)=>{ doc.setFontSize(28);doc.setFont('calibri-light'); doc.setTextColor(0,0,0);}
                ...
                <div className="print-container" style={{ textAlign: 'center' }}>
                    <div className="a4-page">
                        ...
                        <Stack direction="row" alignContent="center" spacing={2}>
                            <div>Date: <span>{form.date.format("DD-MM-YYYY")}</span></div>
                            <Stack direction="column" spacing={2}>
                                <div>Heure alarme: <span>{form.startedAt.format("HH:mm")}</span></div>
                                <div>Heure de fin: <span>{form.endedAt.format("HH:mm")}</span></div>
                            </Stack>
                        </Stack>
                        <Stack direction="row" spacing={2}>
                            <h2>Équipage :</h2>
                            <Stack direction="column" spacing={2}>
                                <div>Pilote: <span>{form.pilote.map(p => p.name).join(', ')}</span></div>
                                <div>Equipiers: <span>{form.crew.map(p => p.name).join(', ')}</span></div>
                            </Stack>
                        </Stack>
                    </div>
                </div>
