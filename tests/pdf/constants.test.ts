...
import { TEXT_FONT, SUBTITLE_FONT, TITLE, HEADER_TITLE } from "../../renderer/components/generation/pdf/constants";
...
describe("constants pdf", () => {
  ...
  it("TEXT_FONT should set the correct font settings for text", () => {
    ...
    expect({fontName, fontStyle}).toEqual({ fontName: 'calibri', fontStyle: 'normal' });
  ...
  it("SUBTITLE_FONT should set the correct font settings for subtitles", () => {
    SUBTITLE_FONT(doc);
    const {fontName, fontStyle} = doc.getFont();
    expect({fontName, fontStyle}).toEqual({ fontName: 'calibri', fontStyle: 'light' });
  ...
  it("TITLE should set the correct font settings for titles", () => {
    TITLE(doc);
    const {fontName, fontStyle} = doc.getFont();
    expect({fontName, fontStyle}).toEqual({ fontName: 'calibri', fontStyle: 'bold' });
