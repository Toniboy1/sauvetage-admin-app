...
describe("Rescued Functionality", () => {
  ...
  it("should add rescue statistics to the document", () => {
    ...
    expect(doc.text).toHaveBeenCalledWith(`Nombre de personne assistées: ${form.rescued}`, 20, startingY + 10);
    expect(doc.text).toHaveBeenCalledWith(`Nombre de personnes médicalisées: ${form.medicalized}`, 20, startingY + 20);
    expect(doc.text).toHaveBeenCalledWith(`Nombre de oersonne décédées: ${form.deceased}`, 100, startingY + 10);
    expect(doc.text).toHaveBeenCalledWith(`Immatriculation du bateau: ${form.boatRegistration}`, 100, startingY + 20);
