...
describe("Time Functionality", () => {
    ...
    it("should add date and time details to the document", () => {
        ...
        expect(doc.text).toHaveBeenCalledWith(`Date: ${form.date.format("DD-MM-YYYY")}`, 20, startingY + 10);
        expect(doc.text).toHaveBeenCalledWith(`Heure alarme: ${form.startedAt.format("HH:mm")}`, 90, startingY + 10);
        expect(doc.text).toHaveBeenCalledWith(`Heure de fin: ${form.endedAt.format("HH:mm")}`, 90, startingY + 20);
